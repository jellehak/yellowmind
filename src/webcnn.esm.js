 ...
 * MIT License
 */
import { GaussianRNG } from './mathutils.js'
...
export const TYPES = {
  ...
  FORWARD_MODE_TRAINING, // Normal training mode
  FORWARD_MODE_EVALUATE, // Normal evaluation mode where no values need to be calculated for backprop
  BACKWARD_MODE_TRAINING,
  BACKWARD_MODE_DREAMING
}
  ...
  newLayer (layerDesc) {
    ...
    if (this.nextLayerIndex === 0) {
      if (newLayer.type !== LAYER_TYPE_INPUT_IMAGE && newLayer.type !== LAYER_TYPE_INPUT) {
        throw new Error('First layer must be input type')
      }
    } else {
...
class InputImageLayer extends Layer {
  constructor (layerName, imageWidth, imageHeight, imageDepth) {
    if (imageDepth !== 1 && imageDepth !== 3) {
      ...
      throw new Error('Invalid input image depth, must be 1 (greyscale) or 3 (RGB)')
    }
...
class MaxPoolLayer extends Layer {
  ...
  forward () {
    ...
    if (intBits > 32) {
      throw new Error('Pool dimensions are too large, max supported size is 5x5')
    }
  ...
  backward () {
    ...
    var example, x, y, ix, iy, id, sx, sy, bit
...
class Array3D {
  ...
  setFromImageData (imageData, depth) {
    ...
    if (depth !== 1 && depth !== 3) {
      throw new Error('Set Array3D from ImageData with unsupported depth')
    }

    if (imageData.width !== this.dimensions.width || imageData.height !== this.dimensions.height || this.dimensions.depth !== depth) {
      throw new Error('Set Array3D from ImageData width, height or depth mismatch')
    }
